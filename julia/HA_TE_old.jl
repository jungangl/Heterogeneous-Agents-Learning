...
## a_vec =  LinRange(a_min, a_max, N) and ϕ_vec
...
function ν̄_f(ai, si, cf, para)
    ...
    ν̄ = (1 + r̄) * (P[si,:]' * [cf[s](ai′) ^ (-σ) for s in 1:S])
    return ν̄
end
...
function indi_labor_supply(para, r, w, ai, si, ψi, xi, cf2)
...
function iterate_agents(para, r, w, a, s, ψ, x, cf, cf2)
    ...
    for i in 1:agent_num
        ai, si, ψi, xi = a[i], s[i], ψ[i, :], x
        ci, ni, ai′ = indi_labor_supply(para, r, w, ai, si, ψi, xi, cf2)[2:end]
...
## 5. prices r, w
function TE(para, a, s, ψ, x, θ, cf)
  @unpack σ, β, α, δ, P, A, agent_num, w̄ = para
  lnw = nlsolve(lnw -> F(lnw, α, θ, δ, para, a, s, ψ, x, cf), [log(w̄)]; inplace = false, iterations = 100_000).zero::Vector{Float64}
  w = exp.(lnw[1])
  k = (w / ((1 - α) * θ)) ^ (1 / α)
  r = α * θ * (k) ^ (α - 1) - δ
  cf2 = get_cf2(para, cf, r, w)
